            ...
            .sheet(isPresented: $showProSheet) {
                NavigationStack {
                    ProUpgradeView(
                        ...
                        featureTitle: "リアクションロール Pro",
                        description: "Proプランではリアクション数が無制限になり、認証モード・永続モードなど高度な設定も利用できます。",
                        flowSteps: [
                            .init("📋", "パネルに絵文字を追加（無制限）"),
                            .init("⚙️", "各絵文字に対してモードを設定",
                                  "通常モード / 認証モード（付与後剥奪不可）/ 永続モード（付与のみ）から選択"),
                            .init("💜", "メンバーがリアクションするとロールが即時反映"),
                        ],
                        useCases: [
                            .init("♾️", "リアクション数を無制限に",
                                  "Freeプランのリアクション上限を超えて、どれだけ多くの絵文字・ロールも自由に設定できます。"),
                            .init("🛡", "認証モードでロールを保護",
                                  "一度付与したロールはリアクションを外しても剥奪されません。プレミアム会員証明などの用途に最適です。"),
                            .init("📌", "永続モードで一方向の付与",
                                  "リアクションを付けたときだけロールが付与され、外しても変化しません。イベント参加登録など、取り消し不可なフローに活用できます。"),
                        ],
                        advancedTips: [
                            "リアクションが多い場合は用途別に複数のパネルに分けると、メンバーが迷わず選択できます",
                            "認証モードを活用してメンバー登録（一方向のロール付与）フローを構築し、参加証明として使えます",
                        ]
                    )
                }
            }
